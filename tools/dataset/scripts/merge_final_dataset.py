#!/usr/bin/env python3
"""
Merge Class 0 (pothole), Class 1 (garbage_pile), Class 2 (waterlogging) 
into final_3class dataset structure for training.

Usage:
    python merge_final_dataset.py \
        --pothole civicshield-dataset/processed/pothole_yolo \
        --garbage civicshield-dataset/processed/custom_garbage \
        --waterlogging civicshield-dataset/processed/waterlogging_unique \
        --output civicshield-dataset/processed/final_3class

This script:
1. Copies all images/labels from three class sources
2. Ensures no filename conflicts (prefixes with class source)
3. Creates the final_3class structure expected by training pipeline
"""
...
def main():
    parser = argparse.ArgumentParser(description="Merge 3-class datasets for final training")
    parser.add_argument("--pothole", type=Path, required=True, help="Pothole dataset (class 0)")
    parser.add_argument("--garbage", type=Path, required=True, help="Garbage dataset (class 1)")
    parser.add_argument("--waterlogging", type=Path, required=True, help="Waterlogging dataset (class 2)")
    parser.add_argument("--output", type=Path, required=True, help="Output final_3class directory")
    
    args = parser.parse_args()
    
    # Verify inputs
    for name, path in [("pothole", args.pothole), ("garbage", args.garbage), ("waterlogging", args.waterlogging)]:
        img_dir = path / "images"
        lbl_dir = path / "labels"
    ...
    # Process each class
    # For pothole and waterlogging, they already have train/ subdirs
    # For garbage, it might be flat (from Roboflow export integration)
    ...
    # Garbage (class 1) - might be flat or have train/val
    garbage_train_img = args.garbage / "images" / "train" if (args.garbage / "images" / "train").exists() else args.garbage / "images"
    garbage_train_lbl = args.garbage / "labels" / "train" if (args.garbage / "labels" / "train").exists() else args.garbage / "labels"
    garbage_val_img = args.garbage / "images" / "val" if (args.garbage / "images" / "val").exists() else None
    garbage_val_lbl = args.garbage / "labels" / "val" if (args.garbage / "labels" / "val").exists() else None
    ...
    for prefix, src_img, src_lbl in [
        ...
        ("garbage", garbage_train_img, garbage_train_lbl),
    ]:
        if src_img.exists() and src_lbl.exists():
            ci, cl = copy_with_prefix(src_img, src_lbl, out_train_img, out_train_lbl, prefix)
            total_train_img += ci
            total_train_lbl += cl
        else:
    ...
    for prefix, src_img, src_lbl in [
        ...
        ("garbage", garbage_val_img, garbage_val_lbl),
    ]:
        if src_img and src_lbl and src_img.exists() and src_lbl.exists():
            ci, cl = copy_with_prefix(src_img, src_lbl, out_val_img, out_val_lbl, prefix)
            total_val_img += ci
            total_val_lbl += cl
        else:
