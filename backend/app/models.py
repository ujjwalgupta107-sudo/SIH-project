...
class Incident(Base):
    __tablename__='incidents'; id:Mapped[str]=mapped_column(String,primary_key=True,default=uid); citizen_id:Mapped[str|None]=mapped_column(ForeignKey('users.id'),nullable=True); type:Mapped[str]=mapped_column(String(100)); description:Mapped[str]=mapped_column(Text); latitude:Mapped[float]=mapped_column(Float); longitude:Mapped[float]=mapped_column(Float); location:Mapped[object]=mapped_column(GeographicPoint()); address:Mapped[str]=mapped_column(String(500)); severity:Mapped[int]=mapped_column(Integer); confidence:Mapped[float]=mapped_column(Float); risk_level:Mapped[str]=mapped_column(String(20)); department:Mapped[str]=mapped_column(String(100)); status:Mapped[str]=mapped_column(String(40),default='REPORTED'); created_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow); updated_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow,onupdate=datetime.utcnow)
...
class IncidentMedia(Base):
    __tablename__='incident_media'; id:Mapped[str]=mapped_column(String,primary_key=True,default=uid); incident_id:Mapped[str]=mapped_column(ForeignKey('incidents.id')); url:Mapped[str]=mapped_column(String); media_type:Mapped[str]=mapped_column(String); stage:Mapped[str]=mapped_column(String,default='BEFORE')
