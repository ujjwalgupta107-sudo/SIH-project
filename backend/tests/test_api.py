...
def test_incident_creation_and_retrieval(client):
    res=client.post('/api/v1/incidents',json=payload(),headers=token('CITIZEN')); assert res.status_code==201
    data=res.json(); assert data['status']=='REPORTED'; assert client.get('/api/v1/incidents/'+data['id'],headers=token('CITIZEN')).status_code==200
def test_incident_validation(client): assert client.post('/api/v1/incidents',json={'type':'x'},headers=token('CITIZEN')).status_code==422
...
def test_development_analysis_is_backend_owned(client):
    created=client.post('/api/v1/incidents',json=payload(),headers=token('CITIZEN')).json()
